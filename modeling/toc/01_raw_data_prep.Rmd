...
source("src/add_column_if_not_exists.R")
...
#Get most recent files
all_data_2023_wide_file <- list.files(path = "data/upper_clp_dss/sensor/prepped/", pattern = "all_2023_sensor_data_.*\\.parquet$", full.names = TRUE)%>%
  sort(decreasing = TRUE)%>%
  .[1]
all_data_2024_wide_file <- list.files(path = "data/upper_clp_dss/sensor/prepped/", pattern = "all_2024_sensor_data_.*\\.parquet$", full.names = TRUE)%>%
  sort(decreasing = TRUE)%>%
  .[1]
all_data_2025_wide_file <- list.files(path = "data/upper_clp_dss/sensor/prepped/", pattern = "all_2025_sensor_data_.*\\.parquet$", full.names = TRUE)%>%
  sort(decreasing = TRUE)%>%
  .[1]
```
...
```{r}
write_parquet(all_data_wide, paste0("data/upper_clp_dss/sensor/prepped/all_sensor_data_wide_", Sys.Date(),".parquet"))
...
water_chem <- bind_rows(ross_water_chem, FC_chem)%>%
  filter(site_code %in% unique(all_data_wide$site))
